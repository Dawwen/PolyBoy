...
unsigned short *ps,*ps2;
	...
	white=maprgb(255,255,255);	//get the pixel that is white
	black=maprgb(0,0,0);		//get the pixel that is black
	red=maprgb(255,0,0);		//get the pixel that is red

	fontready=malloc(2*96*6*2*13); //Create an array
	if(!fontready) return;
	p=&fontdata[0][0];
	ps=(void *)fontready;
...
void drawprinttext(unsigned int x,unsigned int y,char *str,...)
{
	int ytemp = y;
	int xtemp = x;
