...
*/
		case SDL_FINGERUP:
			if(sdlTouchID==event.tfinger.touchId) {
				addcode(MYMOUSE1UP);
				addcode((int)(event.tfinger.x*vxsize)/1932);
				addcode((int)(event.tfinger.y*vysize)/1088);
			}
			break;
		case SDL_FINGERDOWN:
			if(sdlTouchID==event.tfinger.touchId) {
...
void opendisplay(int sx,int sy)	//return -1 if the init didn't work
{
unsigned long videoflags;
int i;
unsigned char r,g,b;	//uint 8

	vxsize=sx;				//Set the width of the screen globally
	vysize=sy;				//Set the width of the screen globally
	codeput=codetake=0;		//type int

	//Start SDL video and joystick functionality
	if ( SDL_Init(SDL_INIT_VIDEO|SDL_INIT_JOYSTICK))
	{
		exit(-1);
	}

//NOT MINE	SDL_CreateWindowAndRenderer(0, 0, SDL_WINDOW_FULLSCREEN_DESKTOP, &sdlWindow, &sdlRenderer);

	//Create the window were the rendering will be
	if ((sdlWindow = SDL_CreateWindow( "SDLLopan", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, sx, sy, SDL_WINDOW_FULLSCREEN_DESKTOP)) == NULL)
		exit(-1);
	...
	//Create a texture taking the whole screen IN rgb 565
    if ((thescreen = SDL_CreateTexture(sdlRenderer,SDL_PIXELFORMAT_RGB565,SDL_TEXTUREACCESS_STREAMING,sx, sy))== NULL)
		exit(-1);

	//Create a surface taking the whole screen
	if ((sdlSurface= SDL_CreateRGBSurfaceWithFormat(0, sx, sy, 16, SDL_PIXELFORMAT_RGB565))== NULL)
		exit(-1);

	//Bind the joystick if found
	if (SDL_NumJoysticks() > 0) {
		// Open joystick
		sdljoy = SDL_JoystickOpen(0);
	} else exit(-1);

	//Bind the touchscreen functionality
	sdlTouchID = SDL_GetTouchDevice(0);

	//Pick the color to draw
	...
	stride= sdlSurface->pitch;			//the length of a row of pixels in bytes (Read only)
	videomem=sdlSurface->pixels;		//define videomem

	darker=malloc(65536*sizeof(unsigned short));	//16 bits unsigned short array (containing lighter color)
	lighter=malloc(65536*sizeof(unsigned short));	//16 bits unsigned short array (containing darker color)

	if(!darker || !lighter || !videomem) 	//not enough memory
		nomem(5);							//return void exit SDL

	for(i=0;i<65536;++i)	//For every color possible
	{
		SDL_GetRGB(i,sdlSurface->format,&r,&g,&b); //Get the RGB composant
		darker[i]=SDL_MapRGB(sdlSurface->format,r*3>>2,g*3>>2,b*3>>2); //darken the color in the tab

		if(r>128 && g>128 && b>128)	//If the pixel can be lightened
			lighter[i]=SDL_MapRGB(sdlSurface->format,(r+255)>>1,(g+255)>>1,(b+255)>>1); //add light version
		else
			lighter[i]=i;																//add the same color
	}
}
...
void closedisplay(void)
{
	...
	//here add free surface and texture
...
void scrlock(void)
{
	...
	if(!locked && SDL_MUSTLOCK(sdlSurface))
	{
...
void copyup(void)
{
	...
	SDL_UpdateTexture(thescreen, NULL, sdlSurface->pixels, sdlSurface->pitch);
	SDL_RenderClear(sdlRenderer);
	SDL_RenderCopy(sdlRenderer, thescreen, NULL, NULL);
	SDL_RenderPresent(sdlRenderer);
...
//	if(i!=1) {close(ihand);return 7;} // 1=color/bw,2=grey
	...
	if(numbpp==8)
	{
		...
		for(i=0;i<256;++i)
		{
			...
			g=pcxci();
			b=pcxci();
			gs->rgb[i]=SDL_MapRGB(sdlSurface->format,r,g,b);
		}
	} else
	{
		for(i=0,j=0;i<16;++i)
...
void rgbdot(unsigned int x,unsigned int y,unsigned char r,unsigned char g,unsigned char b)
{
	if(x<vxsize && y<vysize)
		*(unsigned short *)(videomem+y*stride+x+x)=
			SDL_MapRGB(sdlSurface->format,r,g,b);
}
...
int i;
int c;
	c=SDL_MapRGB(sdlSurface->format,r,g,b);
	if(y<0)
	{
		sizey+=y;
		y=0;
...
{
	return SDL_MapRGB(sdlSurface->format,r,g,b);
}
